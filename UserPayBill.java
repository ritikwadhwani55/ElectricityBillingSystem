...
public class UserPayBill extends JFrame implements ActionListener,ItemListener{
    JPanel jp;
    JButton cancel,pay;
    JLabel bill,total_bill,fixed_rate,service_charge,service_rate,meter_rent,meter_no,units_consumed;
    Choice unpaid_months;
    String email,pswd;
    float cost_per_unit,bill_id;
    int connctn_id;
    public UserPayBill(String username,String password)
    {
        super("Pay a Bill");
        this.email=username;
        this.pswd=password;
...
//        this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        ...
        try
        {
            query="select c.meter_number from connection c inner join customer cust on c.customer_id=cust.customer_id where cust.email='"+this.email+"' and password='"+this.pswd+"'";
            
            rs=c.s.executeQuery(query);
            if(rs.next())
            {
               this.meter_no=new JLabel(rs.getString("meter_number"));
            }
            else
            {
                JOptionPane.showMessageDialog(null, "You have no bills to pay!");
                this.setVisible(false);
            }
            this.meter_no.setBounds(250,50,180,30);
            jp.add(this.meter_no);
//            this.meter_no.addItemListener(this);
...
//            String query=null;
                try {
                        query="SELECT b.* from connection c inner join bill b on b.connection_id=c.connection_id where c.meter_number="+Integer.parseInt((String)this.meter_no.getText())+" and (c.disconnect_date is null or b.payment_status='Unpaid')";
                        rs=c.s.executeQuery(query);
            //            btn=new ButtonGroup();
    ...
    public void actionPerformed(ActionEvent ac)
    {
        ...
        else if(ac.getSource()==pay)
        {
            ...
            {
                JOptionPane.showMessageDialog(null, "Please select correct Month!");
    ...
    public void itemStateChanged(ItemEvent ie)
    {
        ...
        ResultSet rs;
        Conn c=new Conn();
        if(ie.getSource()==this.unpaid_months)
        {
    ...
    {
        new UserPayBill("kamal@gmail.com","kamal");
    }
}
