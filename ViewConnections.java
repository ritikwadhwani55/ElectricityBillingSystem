...
public class ViewConnections extends JFrame{
    ...
    public ViewConnections(String email,String password)
    {
        super("View My Connections");
        this.email=email;
...
//        this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        ...
        p.add(lab);
        String col_names[]={"Customer Name","Meter Number","Connection Address","City","Meter Location","Installation Date","Disconnection Date"};
        ...
        try
        {
            ...
            int c_id=0;
            if(re.next())
                c_id=Integer.parseInt(re.getString("customer_id"));
            sql="select cust.full_name,c.meter_number,c.conn_address,c.city,c.meter_location,c.installation_date,c.disconnect_date from connection c inner join customer cust on c.customer_id=cust.customer_id where cust.customer_id="+c_id;
            re=c.s.executeQuery(sql);
            int r=0,c1=0;
            while(re.next())
            {
    ...
    {
        new ViewConnections("kamal@gmail.com","kamal");
    }
}
