...
public class EditProfile extends JFrame implements ActionListener{
    ...
    JTextArea address;
    JButton edit,cancel;
    public EditProfile(String username,String password)
    {
        super("Edit Profile");
        ...
        doj.setBounds(20,370,180,20);
        doj.setFont(new Font("Tahoma",Font.PLAIN,16));
        p.add(doj);
        JLabel email=new JLabel("Email ID: ");
        email.setBounds(20,410,110,20);
        email.setFont(new Font("Tahoma",Font.PLAIN,16));
        p.add(email);
        JLabel pass=new JLabel("Password: ");
        pass.setBounds(20,450,100,20);
        ...
        Conn conn=new Conn();
        try {
                ResultSet rs=conn.s.executeQuery("select full_name,gender,age,salary,address,contact,doj,email,password from admin where email='"+this.username+"' and password='"+this.password+"'");
                if(rs.next())
                {
                    ...
                    this.contact.setBounds(200,220,150,20);
                    p.add(this.contact);
                    ...
                    JLabel salary1=new JLabel(rs.getString("salary"));
                    salary1.setBounds(200,180,150,20);
                    salary1.setFont(new Font("Tahoma",Font.PLAIN,16));
                    p.add(salary1);
                    
                    this.address=new JTextArea();
                    this.address.setBounds(200,260,350,90);
                    p.add(this.address);
                    this.address.setFont(new Font("Tahoma",Font.PLAIN,16));
                    this.address.setText(rs.getString("address"));
                    
                    JLabel doj1=new JLabel(rs.getString("doj"));
                    doj1.setBounds(200,370,150,20);
                    doj1.setFont(new Font("Tahoma",Font.PLAIN,16));
                    p.add(doj1);
                    
                    JLabel email1=new JLabel(rs.getString("email"));
                    email1.setBounds(200,410,150,20);
                    email1.setFont(new Font("Tahoma",Font.PLAIN,16));
                    p.add(email1);
                    
                    JLabel passwd=new JLabel();
                    passwd.setBounds(200,450,150,20);
                    p.add(passwd);
                    passwd.setFont(new Font("Tahoma",Font.PLAIN,16));
                    passwd.setText(rs.getString("password"));
                    
                    edit=new JButton("Edit");
                    this.edit.setBounds(20,495,150,30);
                    p.add(this.edit);
                    ...
                    this.cancel.setBounds(200,495,150,30);
                    p.add(this.cancel);
...
//        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
    ...
    public void actionPerformed(ActionEvent action)
    {
        if(action.getSource()==edit)
        {
             Conn conn=new Conn();
            try {
                String qu="update admin set age="+Integer.parseInt(this.age.getText())+",contact="+Long.parseLong(this.contact.getText())+",address='"+this.address.getText()+"' where email='"+this.username+"' and password='"+this.password+"'";
    ...
    {
        new EditProfile("ritik@gmail.com","ritik");
    }
}
